/*
  相关：https://github.com/dtrebbien/BigDecimal.js/blob/master/build/BigDecimal-all-last.js
  相关：https://github.com/MikeMcl/big.js
 */
(function() {
  ...
  */
        ...
        function MathContext() {
            ...
            if (setdigits != this.DEFAULT_DIGITS) {
                if (setdigits < this.MIN_DIGITS) throw "MathContext(): Digits too small: " + setdigits;
                if (setdigits > this.MAX_DIGITS) throw "MathContext(): Digits too large: " + setdigits;
            } {
                /*select*/
                if (setform == this.SCIENTIFIC) {} // [most common]
                else if (setform == this.ENGINEERING) {} else if (setform == this.PLAIN) {} else {
                    throw "MathContext() Bad form value: " + setform;
                }
            }
            if ((! (this.isValidRound(setroundingmode)))) throw "MathContext(): Bad roundingMode value: " + setroundingmode;
  ...
  */
        ...
        function toString() {
            ...
            var roundword = null; {
                /*select*/
                if (this.form == this.SCIENTIFIC) formstr = "SCIENTIFIC";
                else if (this.form == this.ENGINEERING) formstr = "ENGINEERING";
                else {
                    formstr = "PLAIN";
                    /* form=PLAIN */
                }
        ...
        function isValidRound(testround) {
            ...
            var r = 0; {
                ...
                r = 0;
                r: for (; $2 > 0; $2--, r++) {
                    if (testround == this.ROUNDS[r]) return true;
                }
            }
            /*r*/
            return false;
 ...
 */
  ...
  */
        ...
        /* properties constant private unused */
  ...
  */
        ...
        function BigDecimal() {
            ...
            //-- overloaded ctor
            if (BigDecimal.arguments.length == 0) return;
            var inchars;
            var offset;
            var length;
            ...
            if (length <= 0) this.bad("BigDecimal(): ", inchars); // bad conversion (empty string)
            // [bad offset will raise array bounds exception]
            /* Handle and step past sign */
            this.ind = this.ispos; // assume positive
            if (inchars[0] == ('-')) {
                length--;
                if (length == 0) this.bad("BigDecimal(): ", inchars); // nothing after sign
                this.ind = this.isneg;
                offset++;
            } else if (inchars[0] == ('+')) {
                length--;
            ...
            last = -1; // last character of mantissa
            {
                ...
                i: for (; $1 > 0; $1--, i++) {
                    ...
                    if (si == '.') { // record and ignore
                        if (dotoff >= 0) this.bad("BigDecimal(): ", inchars); // two dots
                        dotoff = i - offset; // offset into mantissa
                        continue i;
                    }
                    if (si != 'e') if (si != 'E') { // expect an extra digit
                        if (si < '0' || si > '9') this.bad("BigDecimal(): ", inchars); // not a number
                    ...
                    if ((i - offset) > (length - 2)) this.bad("BigDecimal(): ", inchars); // no room for even one digit
                    eneg = false;
                    if ((inchars[i + 1]) == ('-')) {
                        eneg = true;
                        k = i + 2;
                    ...
                    if ((elen == 0) || (elen > 9)) this.bad("BigDecimal(): ", inchars); // 0 or more than 9 digits
                    {
                        var $2 = elen;
                        j = k;
                        j: for (; $2 > 0; $2--, j++) {
                            sj = inchars[j];
                            if (sj < '0') this.bad("BigDecimal(): ", inchars); // always bad
      ...
      if (dvalue<0)
       bad(inchars); // not base 10*/
                    ...
                    /*j*/
                    if (eneg) this.exp = -this.exp; // was negative
            ...
            if (d == 0) this.bad("BigDecimal(): ", inchars); // no mantissa digits
            if (dotoff >= 0) this.exp = (this.exp + dotoff) - d; // adjust exponent if had dot
                ...
                exotica: do { // slow: check for exotica
                    {
                        ...
                        i = 0;
                        i: for (; $4 > 0; $4--, i++) {
                            if (i == dotoff) j++; // at dot
                            sj = inchars[j];
                            if (sj <= '9') this.mant[i] = sj - '0';
                            /* easy */
                            else {
            ...
            if (this.mant[0] == 0) {
                ...
                if (this.exp > 0) this.exp = 0; // positive exponent can be ignored
                ...
                if (hadexp) {
                    ...
                    if ((mag < this.MinExp) || (mag > this.MaxExp)) this.bad("BigDecimal(): ", inchars);
                }
            }
  ...
  */
        ...
        function abs() {
            ...
            if (this.ind == this.isneg) return this.negate(set);
  ...
  */
        ...
        function add() {
            ...
            if (set.lostDigits) this.checkdigits(rhs, set.digits);
            lhs = this; // name for clarity and proxy
            /* Quick exit for add floating 0 */
            // plus() will optimize to return same object if possible
            if (lhs.ind == 0) if (set.form != MathContext.prototype.PLAIN) return rhs.plus(set);
            if (rhs.ind == 0) if (set.form != MathContext.prototype.PLAIN) return lhs.plus(set);

            /* Prepare numbers (round, unless unlimited precision) */
            reqdig = set.digits; // local copy (heavily used)
            if (reqdig > 0) {
                if (lhs.mant.length > reqdig) lhs = this.clone(lhs).round(set);
                if (rhs.mant.length > reqdig) rhs = this.clone(rhs).round(set);
     ...
     still need to pad with up to DIGITS trailing zeros]. */
       ...
       need to pad up to a length of DIGITS+1. */
                        if (newlen >= ((userlen + reqdig) + 1)) if (reqdig > 0) {
                            // LHS is sufficient
                            res.mant = usel;
                            res.exp = lhs.exp;
                            res.ind = lhs.ind;
                        ...
                        if (newlen > (reqdig + 1)) if (reqdig > 0) {
                        ...
                        if (newlen > usellen) usellen = newlen; // need to pad LHS
                    } else { // need to pad rhs and/or truncate lhs
                        newlen = (userlen + rhs.exp) - lhs.exp;
                        if (newlen >= ((usellen + reqdig) + 1)) if (reqdig > 0) {
                            // RHS is sufficient
                            res.mant = user;
                            res.exp = rhs.exp;
                            res.ind = rhs.ind;
                        ...
                        if (newlen > userlen) userlen = newlen; // need to pad RHS
                    }
                } while ( false );
            }
            /*padder*/
            ...
            if (lhs.ind == this.iszero) res.ind = this.ispos;
            else res.ind = lhs.ind; // likely sign, all paths
       ...
       so we may need to swap the operands. */
                    {
                        swaptest: do {
                            /*select*/
                            if (rhs.ind == this.iszero) {} // original A bigger
                            ...
                            } else if (usellen > userlen) {} // original A bigger
                            else {
                                {
                                    /* logical lengths the same */
                                    // need compare
                                    ...
                                    eb = user.length - 1; {
                                        compare: for (;;) {
                                            if (ia <= ea) ca = usel[ia];
                                                ...
                                                if (ib > eb) {
                                                    /* identical */
                                                    if (set.form != MathContext.prototype.PLAIN) return this.ZERO;
  ...
  */
        ...
        function divide() {
            ...
            if (divide.arguments.length == 2) {
                ...
                if (scale < 0) throw "divide(): Negative scale: " + scale;
  ...
  */
        ...
        function multiply() {
            ...
            if (reqdig > 0) {
                ...
                if (lhs.exp > 0) padding = padding + lhs.exp;
                if (rhs.exp > 0) padding = padding + rhs.exp;
            }
            ...
            if ((multer[0] * multand[0]) > 9) acclen = multandlen + 1;
            else acclen = multandlen;
            ...
            if (padding == 0) res.mant = acc;
            else res.mant = this.extend(acc, acc.length + padding); // add trailing 0s
            return res.finish(set, false);
        }
  ...
  */
        ...
        function negate() {
            ...
            if (set.lostDigits) this.checkdigits(null, set.digits);
  ...
  */
        ...
        function plus() {
            ...
            if (set.form == MathContext.prototype.PLAIN) if (this.form == MathContext.prototype.PLAIN) {
                if (this.mant.length <= set.digits) return this;
                if (set.digits == 0) return this;
            }
  ...
  */
        ...
        function pow() {
            ...
            if (n < 0) n = -n; // [rhs.ind records the sign]
  ...
  */
        ...
        function equals(obj) {
            ...
            if (obj == null) return false; // not equal
            if ((! (((obj instanceof BigDecimal))))) return false; // not a decimal
            rhs = obj; // cast; we know it will work
            if (this.ind != rhs.ind) return false; // different signs never match
            ...
            { // mantissas say all
                ...
                rca = rhs.layout();
                if (lca.length != rca.length) return false; // mismatch
  ...
  */
        ...
        function format() {
            ...
            /* Check arguments */
            if ((before < ( - 1)) || (before == 0)) this.badarg("format", 1, before);
            if (after < ( - 1)) this.badarg("format", 2, after);
            if ((explaces < ( - 1)) || (explaces == 0)) this.badarg("format", 3, explaces);
            if (exdigits < ( - 1)) this.badarg("format", 4, exdigits); {
                /*select*/
                if (exformint == MathContext.prototype.SCIENTIFIC) {} else if (exformint == MathContext.prototype.ENGINEERING) {} else if (exformint == ( - 1)) exformint = MathContext.prototype.SCIENTIFIC;
            ...
            if (exround != this.ROUND_HALF_UP) {
                try { // if non-default...
                    if (exround == ( - 1)) exround = this.ROUND_HALF_UP;
                    else new MathContext(9, MathContext.prototype.SCIENTIFIC, false, exround);
                } catch($10) {
  ...
  */
            ...
            /* determine form */
            {
                setform: do {
                    /*select*/
                    if (exdigits == ( - 1)) num.form = MathContext.prototype.PLAIN;
                    else if (num.ind == this.iszero) num.form = MathContext.prototype.PLAIN;
                    else {
                        // determine whether triggers
                        mag = num.exp + num.mant.length;
                        if (mag > exdigits) num.form = exformint;
                        else if (mag < ( - 5)) num.form = exformint;
                        else num.form = MathContext.prototype.PLAIN;
                    }
                } while ( false );
            }
            /*setform*/
     ...
     with 10.0 if scientific). */
            if (after >= 0) {
                setafter: for (;;) {
                    ...
                    {
                        /*select*/
                        if (num.form == MathContext.prototype.PLAIN) thisafter = -num.exp; // has decimal part
                        else if (num.form == MathContext.prototype.SCIENTIFIC) thisafter = num.mant.length - 1;
                        else { // engineering
                            lead = (((num.exp + num.mant.length) - 1)) % 3; // exponent to use
                            if (lead < 0) lead = 3 + lead; // negative exponent case
                            lead++; // number of leading digits
                            if (lead >= num.mant.length) thisafter = 0;
                            else thisafter = num.mant.length - lead;
                        }
                    }
                    if (thisafter == after) break setafter; // we're in luck
                    if (thisafter < after) { // need added trailing zeros
                        ...
                        if (num.exp < this.MinExp) throw "format(): Exponent Overflow: " + num.exp;
                        break setafter;
                    }
                    ...
                    if ((num.exp - oldexp) == chop) break setafter; // number did not have carry
            ...
            if (before > 0) {
                ...
                if (p > before) this.badarg("format", 1, before); // won't fit
            ...
            if (explaces > 0) {
                ...
                if (p == 0) { // no E part; add trailing blanks
                    ...
                    if (places > explaces) this.badarg("format", 3, explaces);
  ...
  */
        ...
        function intValueExact() {
            ...
            if (this.ind == this.iszero) return 0; // easy, and quite common
            ...
            if (this.exp < 0) {
                ...
                if ((! (this.allzero(this.mant, lodigit + 1)))) throw "intValueExact(): Decimal part non-zero: " + this.toString();
            ...
            result = 0; {
                ...
                i: for (; i <= $16; i++) {
                    result = result * 10;
                    if (i <= lodigit) result = result + this.mant[i];
                }
            }
            /*i*/
            ...
            if ((lodigit + useexp) == 9) {
                ...
                if (topdig != this.mant[0]) { // digit must match and be positive
                    ...
                    if (this.mant[0] == 2) return result; // really had top digit 2
                    throw "intValueExact(): Conversion overflow: " + this.toString();
                }
            }

            /* Looks good */
            if (this.ind == this.ispos) return result;
            return - result;
        }
  ...
  */
        ...
        //--public int scale(){
        function scale() {
            if (this.exp >= 0) return 0; // scale can never be negative
            return - this.exp;
        }
  ...
  */
        ...
        function setScale() {
            ...
            if (ourscale <= scale) { // simply zero-padding/changing form
                ...
                if (ourscale == 0) padding = res.exp + scale;
                else padding = scale - ourscale;
  ...
  */
    ...
    <p>'form' describes whether we are to use exponential notation (and
       if so, which), or if we are to lay out as a plain/pure numeric.
        ...
        function layout() {
            ...
            if (this.exp == 0) {
                /* easy */
                if (this.ind >= 0) return cmant; // non-negative integer
    ...
    <p>If OK, returns it as an int. </sgml> */
        ...
        function intcheck(min, max) {
            ...
            if ((i < min) || (i > max)) throw "intcheck(): Conversion overflow: " + i;
            return i;
        }
  ...
  */
        ...
        function dodivide(code, rhs, set, scale) {
            ...
            if (rhs.ind == 0) throw "dodivide(): Divide by 0"; // includes 0/0
            ...
            if (reqdig > 0) {
                ...
                if (scale != ( - lhs.exp)) reqdig = (reqdig + scale) + lhs.exp;
                reqdig = (reqdig - ((rhs.mant.length - 1))) - rhs.exp; // reduce by RHS effect
                if (reqdig < lhs.mant.length) reqdig = lhs.mant.length; // clamp
                if (reqdig < rhs.mant.length) reqdig = rhs.mant.length; // ..
            }
            ...
            if (newexp < 0) if (code != 'D') {
                if (code == 'I') return this.ZERO; // easy - no integer part
            ...
            b2b = (var2[0] * 10) + 1;
            if (var2.length > 1) b2b = b2b + var2[1];
            ...
            have = 0; {
                outer: for (;;) {
                    ...
                    {
                        inner: for (;;) {
                            if (var1len < var2len) break inner; // V1 too low
                                    ...
                                    compare: do { // comparison
                                        {
                                            ...
                                            i: for (; $22 > 0; $22--, i++) {
                                                ...
                                                if (i < var2.length) v2 = var2[i];
                                                else v2 = 0;
                                                if (var1[i] < v2) break inner; // V1 too low
                                                if (var1[i] > v2) break compare; // OK to subtract
                                            }
                                        }
                                        /*i*/
          ...
          being checked) */
                                        thisdigit ++; res . mant [ have ] = thisdigit;
                                        have++;
      ...
      the next digit */
            ...
            if (have == 0) have = 1; // res.mant[0] is 0; we always want a digit
            if ((code == 'I') || (code == 'R')) {
                /* check for integer overflow needed */
                if ((have + res.exp) > reqdig) throw "dodivide(): Integer overflow";
                ...
                if (code == 'R') {
                    remainder: do {
                        ...
                        if (var1[0] == 0) return this.ZERO; // simple 0 residue
         ...
         the resulting mantissa if need be */
                ...
                if (var1[0] != 0) { // residue not 0
                    ...
                    if (((lasthave % 5)) == 0) res.mant[have - 1] = (lasthave + 1);
                }
            }
            ...
            if (have == res.mant.length) { // got digits+1 digits
                ...
                /* have<=reqdig */
                if (res.mant[0] == 0) return this.ZERO; // fastpath
    ...
    */
        ...
        function extend(inarr, newlen) {
            //--byte newarr[];
            var newarr;
            if (inarr.length == newlen) return inarr;
    ...
    */
        ...
        function byteaddsub(a, avlen, b, bvlen, m, reuse) {
            ...
            bp = bvlen - 1; // ..
            maxarr = bp;
            if (maxarr < ap) maxarr = ap;
            reb = null; // result byte array
            if (reuse) if ((maxarr + 1) == alength) reb = a; // OK to reuse A
            if (reb == null) {
                reb = this.createArrayWithZeros(maxarr + 1); // need new array
            }

            quickm = false; // 1 if no multiply needed
            if (m == 1) quickm = true; // most common
            else if (m == ( - 1)) quickm = true; // also common
            digit = 0; // digit, with carry or borrow
            {
                op = maxarr;
                op: for (; op >= 0; op--) {
                    if (ap >= 0) {
                        if (ap < alength) digit = digit + a[ap]; // within A
                        ap--;
                    }
                    if (bp >= 0) {
            ...
            newarr = null;
            if (reuse) if ((maxarr + 2) == a.length) newarr = a; // OK to reuse A
            if (newarr == null) newarr = new Array(maxarr + 2);
    ...
    */
    ...
    returns quietly or throws an exception */
        ...
        function checkdigits(rhs, dig) {
            if (dig == 0) return; // don't check if digits=0
            // first check lhs...
            if (this.mant.length > dig) if ((! (this.allzero(this.mant, dig)))) throw "Too many digits: " + this.toString();
            if (rhs == null) return; // monadic
            if (rhs.mant.length > dig) if ((! (this.allzero(rhs.mant, dig)))) throw "Too many digits: " + rhs.toString();
            return;
        }
    ...
    */
        ...
        function round() {
            ...
            if (adjust <= 0) return this; // nowt to do
            ...
            if (len > 0) {
                ...
                if (len == 0) first = oldmant[0];
                else first = 0; // [virtual digit]
            }
            ...
            increment = 0; // bumper
            {
                modes: do {
                    ...
                    if (mode == this.ROUND_HALF_UP) { // default first [most common]
                        if (first >= 5) increment = sign;
                    } else if (mode == this.ROUND_UNNECESSARY) { // default for setScale()
                        // discarding any non-zero digits is an error
                        if ((! (this.allzero(oldmant, len)))) throw "round(): Rounding necessary";
                    } else if (mode == this.ROUND_HALF_DOWN) { // 0.5000 goes down
                        if (first > 5) increment = sign;
                        else if (first == 5) if ((! (this.allzero(oldmant, len + 1)))) increment = sign;
                    } else if (mode == this.ROUND_HALF_EVEN) { // 0.5000 goes down if left digit even
                        if (first > 5) increment = sign;
                        else if (first == 5) {
                            if ((! (this.allzero(oldmant, len + 1)))) increment = sign;
                            else
                            /* 0.5000 */
                            if ((((this.mant[this.mant.length - 1]) % 2)) == 1) increment = sign;
                        }
                    } else if (mode == this.ROUND_DOWN) {} // never increment
                    else if (mode == this.ROUND_UP) { // increment if discarded non-zero
                        if ((! (this.allzero(oldmant, len)))) increment = sign;
                    } else if (mode == this.ROUND_CEILING) { // more positive
                        if (sign > 0) if ((! (this.allzero(oldmant, len)))) increment = sign;
            ...
            if (increment != 0) {
                bump: do {
                    if (this.ind == this.iszero) {
                        ...
                        if (this.ind == this.isneg) increment = -increment;
   ...
   */
        ...
        function finish(set, strip) {
            ...
            if (set.digits != 0) if (this.mant.length > set.digits) this.round(set);

            /* If strip requested (and standard formatting), remove
     insignificant trailing zeros. */
            if (strip) if (set.form != MathContext.prototype.PLAIN) {
                d = this.mant.length;
                /* see if we need to drop any trailing zeros */
                {
                    i = d - 1;
                    i: for (; i >= 1; i--) {
                        if (this.mant[i] != 0) break i;
                        d--;
                        this.exp++;
                    }
                }
                ...
                i: for (; $26 > 0; $26--, i++) {
                    if (this.mant[i] != 0) {
                        ...
                        if (mag > 0) { // most common path
                            if (mag > set.digits) if (set.digits != 0) this.form = set.form;
                            if ((mag - 1) <= this.MaxExp) return this; // no overflow; quick return
                        } else if (mag < ( - 5)) this.form = set.form;
                        /* check for overflow */
                        mag--;
                        if ((mag < this.MinExp) || (mag > this.MaxExp)) {
                            overflow: do {
                                ...
                                if (this.form == MathContext.prototype.ENGINEERING) {
                                    ...
                                    if (mag >= this.MinExp) if (mag <= this.MaxExp) break overflow;
                                }
            ...
            this.ind = this.iszero; {
                /*select*/
                if (set.form != MathContext.prototype.PLAIN) this.exp = 0; // standard result; go to '0'
                else if (this.exp > 0) this.exp = 0; // +ve exponent also goes to '0'
                else {
                    // a plain number with -ve exponent; preserve and check exponent
                    if (this.exp < this.MinExp) throw "finish(): Exponent Overflow: " + this.exp;
                }
            }
