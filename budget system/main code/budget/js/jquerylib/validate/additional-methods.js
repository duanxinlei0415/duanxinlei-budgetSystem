					...
					.addMethod(
							...
							function(value, element) {
								...
								if (re.test(value)) {
									...
									xdata = new Date(aaaa, mm - 1, gg, 12, 0,
											0, 0);
...
$.validator.addMethod("isPhone", function(value, element) {
	...
	return this.optional(element) || (mobile.test(value) && length === 11);
...
$.validator.addMethod("isPassport", function(value,element) {
    var passport = new RegExp("/(P/d{7})|(G/d{8})/");
    return this.optional(element) || passport.test(value);
}, "请正确填写您的护照号");

isNullOther = function(value,param){
	var target = $( param );
	value = $.trim(value);
	if(target.val() == "其他领域" && value==""){
		return false;
	}else if(target.val() == ""){
		return false;
	}else{
...
$.validator.addMethod("isNullOther", function(value, element, param) {
	return isNullOther(value,param)  ;
}, "请在其他领域中填写或选择技术领域");
...
isNullReturn = function(value,param){
	var target = $( param );
	if(target.val() == "是" && value==""){
		return false;
	}else{
		return true;
	}
}
$.validator.addMethod("isNullReturn", function(value, element, param) {
	return this.optional(element) && isNullReturn(value,param);
}, "请填写归国年份");
